...
from src.model import BaseRGCN, GatingRouter
from src.decoder import *
from unirec.unisrec import UniSRec, MoEAdaptorLayer
from unirec.data.dataset import PretrainUniSRecDataset
...
class RecurrentRGCN(nn.Module):
    ...
    def forward(self, g_list, static_graph, use_cuda,knowledgeseq):
        gate_list = []
        degree_list = []
        ...
        if self.use_static:
            ...
            """
            """
            props = ['../unirec/props/UniSRec.yaml', '../unirec/props/pretrain.yaml']
            config = Config(model=UniSRec, dataset='FHCKM', config_file_list=props, config_dict={})
            MoE_model = MoEAdaptorLayer(config['n_exps'],config['adaptor_layers'],config['adaptor_dropout_prob']).cuda()
            knowledgeseq = knowledgeseq.cuda()
            coledge_emb = MoE_model(knowledgeseq)
            # gr = GatingRouter(768, 200, 1)
            # coledge_emb = gr(knowledgeseq)
            # sliced_features = features[span(0):span(1),:]
            # coledge_emb = static_emb
        ...
        return history_embs, static_emb, self.h_0, gate_list, degree_list,coledge_emb


    def predict(self, test_graph, num_rels, static_graph, test_triplets, entity_history_vocabulary, rel_history_vocabulary, use_cuda, knowledgeseq):
        ...
        with torch.no_grad():
            ...
            evolve_embs, _, r_emb, _, _, _ = self.forward(test_graph, static_graph, use_cuda, knowledgeseq)
    ...
    def get_loss(self, glist, triples, static_graph, entity_history_vocabulary, rel_history_vocabulary, use_cuda, knowledgeseq):
        self.use_cuda = use_cuda
        ...
        evolve_embs, static_emb, r_emb, _, _,coledge_emb = self.forward(glist, static_graph, use_cuda,knowledgeseq)
        ...
        if self.relation_prediction:
            ...
            # if  torch.isnan(score_rel_h).any() :
            #     print("a")
            ...
            if self.discount == 1:
                for time_step, evolve_emb in enumerate(evolve_embs):
                    ...
                    if self.layer_norm:
                        ...
                        """
                        """
                        sim_matrix = torch.sum((static_emb + coledge_emb)* F.normalize(evolve_emb), dim=1)
                        ...
                        """
                        """
                        sim_matrix = torch.sum((static_emb * coledge_emb)* evolve_emb, dim=1)
    ...
    def knowledge_mode(self, pre_emb, r_emb, time_embs, all_triples, history_vocabulary):
        if self.use_cuda:
            knowledge_index = torch.Tensor(np.array(history_vocabulary.cpu(), dtype=float))
            knowledge_index = knowledge_index.to('cuda')
        else:
            knowledge_index = torch.Tensor(np.array(history_vocabulary.cpu(), dtype=float))
        score_knowledge = self.decoder_ob3.forward(pre_emb, r_emb, time_embs, all_triples, partial_embeding = knowledge_index)
        score_k = score_knowledge
        score_k = F.softmax(score_k, dim=1)
        return score_k
